...
/**
 * Gets the location of tweet by all 3 means and then fwds the State code with
 * the tweet to the next Bolt. There are three different objects within a tweet
 * that we can use to determine it’s origin. This Class utilizes all the three
 * of them and prioritizes in the following order [high to low]: 1. The
 * coordinates object 2. The place object 3. The user object
 *
...
public final class StateLocatorBolt extends BaseRichBolt {
    ...
    /**
     * Tries to get the State of the tweet by checking first GeoLocation Object,
     * then Place Object and finally User Object.
     *
...
//            state = Constants.MAP_LAT_LONG_CODE_NAME.get(latitude + "," + longitude);
//        } else {
//            return Optional.absent();
//        }
...
//                System.out.println(status.getPlace().getStreetAddress());
//                System.out.println(status.getPlace().getContainedWithIn()); 
        System.out.println(status.getText());
        System.out.println(status.getPlace().getName());
        System.out.println("PLACE FULL:::" + status.getPlace().getFullName());
        ...
        System.out.println("getStae::" + state);
        return state;
    }

    /**
     * Retrieves the State from GeoLocation Object of the Tweet. This is
     * considered as the primary and correct value for the State of the tweet.
...
//				LOGGER.debug("State from BingMaps:{}", stateFromGeoLocation);
                ...
                System.out.println(stateFromGeoLocation);
            }
        }
        return state;
    }
